...
impl IpIdea {
    pub fn new(cfg: &ProxyConfig) -> Self {
        // http://api.proxy.ipidea.io/getProxyIp?num=100&return_type=json&lb=1&sb=0&flow=1&regions=&protocol=http
...
impl IpPoolServices for IpIdea {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_ip_idea() {
        // "api.proxy.ipidea.io".to_string(), 80
        let ip_idea = IpIdea::new(&ProxyConfig{
            name: "".to_string(),
            password: "".to_string(),
        ...
        let ip = ip_idea.get_ip().await.unwrap();
        println!("ip: {}", ip);
        assert!(true)
    }
}